...
2. [Project Structure](#2-project-structure)
   - 2.1 [Content vs. Style Separation](#21-content-vs-style-separation)
3. [Content Rules](#3-content-rules)
   - 3.1 [Grounding and Traceability](#31-grounding-and-traceability)
   - 3.2 [Audience](#32-audience)
     - 3.2.1 [No native-speaker-only idioms](#321-no-native-speaker-only-idioms)
     - 3.2.2 [Plain-English rephrasing for statistical claims](#322-plain-english-rephrasing-for-statistical-claims)
4. [Style and Structure Rules](#4-style-and-structure-rules)
   - 4.1 [Entry Point](#41-entry-point)
   - 4.2 [Sections (`sections/`)](#42-sections-sections)
   - 4.3 [Figures (`figures/`)](#43-figures-figures)
   - 4.4 [Tables (`tables/`)](#44-tables-tables)
   - 4.5 [Float Placement (Main Text Only)](#45-float-placement-main-text-only)
   - 4.6 [Bibliography (`zotero.bib`)](#46-bibliography-zoterobib)
   - 4.7 [Reference Materials (`resources/`)](#47-reference-materials-resources)
   - 4.8 [Notes (`notes/`)](#48-notes-notes)
   - 4.9 [Fallback (`fallback/`)](#49-fallback-fallback)
   - 4.10 [Literature Search](#410-literature-search)
   - 4.11 [Numerical Precision](#411-numerical-precision)
   - 4.12 [Venue-Aware Structure and Signposting](#412-venue-aware-structure-and-signposting)
     - 4.12.1 [Option A: ACM-Style](#4121-option-a-acm-style-acmart-hci-vis-se-venues)
     - 4.12.2 [Option B1: ARR Venues](#4122-option-b1-arr-venues-acl--emnlp--naacl)
     - 4.12.3 [Option B2: COLM](#4123-option-b2-colm)
     - 4.12.4 [Pure-ML Venues](#4124-pure-ml-venues-iclr--neurips--icml)
   - 4.13 [Writing Style](#413-writing-style)
...
### 2.1 Content vs. Style Separation
...
### 3.1 Grounding and Traceability
...
Assume a typical reader with this profile:

- **Impatient.** Will skim past or close any paragraph that does not pay off within two sentences. Lead with the claim; defer the buildup.
- **Non-native English speaker** with solid technical English, but no fluency in idiomatic or metaphorical usage of common words. Sentences whose meaning depends on knowing what "edge out", "sweet spot", or "moving the needle" mean lose half the audience.
- **Shaky on advanced statistics.** Treats `p < 0.05` as "good" without parsing what the p-value actually quantifies. Does not know the textbook distinctions between Bayesian inference and frequentist hypothesis testing, the assumptions behind a goodness-of-fit test, or what "robust to heteroskedasticity" implies for the conclusion. Statistical claims must be rephrased in plain English alongside (or instead of) the technical form.

Two enforceable rules follow from this profile. Both are load-bearing for every section of the paper.

#### 3.2.1 No native-speaker-only idioms

Replace any common English word used in an idiomatic or metaphorical sense that a non-native reader might miss. The word itself is familiar; the figurative meaning is the problem. Always substitute the literal equivalent:
...
#### 3.2.2 Plain-English rephrasing for statistical claims
...
### 4.5 Float Placement (Main Text Only)
...
### 4.7 Reference Materials (`resources/`)

- `resources/` is **read-only**. It contains reference materials (source code, documentation, blogs, notes) to consult.
- **Never copy and paste from `resources/` verbatim.** When you need to use information from it, read and understand the content, then write your own version in your own words.
...
### 4.12 Venue-Aware Structure and Signposting
...
#### 4.12.1 Option A: ACM-Style (acmart, HCI, vis, SE venues)
...
#### 4.12.2 Option B1: ARR Venues (ACL / EMNLP / NAACL)
...
- **Related Work is optional.** About **8% of ARR papers omit Related Work entirely**, folding the prose into the introduction or a Background section. Reviewers do not penalize this. When present, the concision rule under §4.13 (*Writing Style*) applies.
...
#### 4.12.4 Pure-ML Venues (ICLR / NeurIPS / ICML)
...
- **No "pipeline" for agentic systems.** The word "pipeline" implies a fixed, deterministic sequence of steps. When describing agentic systems (which involve iterative, non-deterministic tool use), use "workflow," "loop," or "system" instead.
...
- **Takeaway signposting:** see §4.12 (*Venue-Aware Structure and Signposting*). ACM-style: explicit call-out boxes. NLP-style: `\paragraph{Takeaway.}` or `In summary, …`. Never both in the same paper.
...
- **No native-speaker-only idioms; plain-English rephrasing of statistical claims.** See §3.2 (*Audience*). Both rules apply to every section, not just the Introduction or the abstract.
- **Avoid AI-favored words and phrases.** The following are statistically overrepresented in AI-generated text and should be used sparingly or replaced with plainer alternatives. This is not a ban; use them when they are genuinely the best word, but prefer simpler, more direct phrasing:
  ...
  - **Structural tells:** "provide a comprehensive," "a nuanced understanding," "the complex interplay," "play a pivotal/crucial role," "a multi-faceted approach," "pave the way," "shed light on," "navigate the complex," "far-reaching implications," "a significant milestone." These are clichés; find concrete language instead.
  - **Buzzwords to watch:** "cutting-edge," "innovative," "seamless integration," "scalable solution." Either drop them or replace with specifics.
